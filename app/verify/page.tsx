...
export default function VerifyPage() {
  ...
  async function handleSign() {
    if (!message.trim()) {
      setError("Paste the message from Telegram bot first.");
      return;
    }
    setError("");
    setSigning(true);
    ...
    <div className="min-h-[calc(100vh-3.5rem)] bg-surface-muted">
      <div className="max-w-xl mx-auto px-6 py-16">
        ...
        <p className="text-[15px] text-text-2 mb-8">
          Sign a message from the Telegram bot to link your wallet.
        </p>

        <div className="space-y-5">
          <div className="bg-surface border border-border p-5">
            <label className="text-[13px] font-semibold text-text mb-2 block">
              1. Paste the message from Telegram
            </label>
            <textarea
              value={message}
              onChange={(e) => { setMessage(e.target.value); setError(""); }}
              placeholder={`Sign this message to verify you own this wallet for Vaxa Telegram Bot.\n\nWallet: 0x...\nTelegram ID: ...\nNonce: vaxa-verify-...`}
              rows={8}
              className="w-full bg-bg border border-border text-text text-[13px] font-mono p-3 focus:outline-none focus:border-border-strong resize-none"
            />
          </div>
          ...
          <div className="bg-surface border border-border p-5">
            ...
            {!isConnected ? (
              <div className="flex items-center gap-4">
                ...
                <span className="text-[13px] text-text-3">MetaMask, Rabby, or any wallet</span>
              </div>
            ) : (
              <button
                onClick={handleSign}
          ...
          {signature && (
            <div className="bg-surface border border-accent/30 p-5">
              <label className="text-[13px] font-semibold text-accent mb-2 block">
                3. Copy this signature and send to Telegram bot:
              </label>
          ...
          <div className="bg-bg border border-border p-4 text-[13px] text-text-3 leading-relaxed">
            ...
            <ol className="mt-2 space-y-1.5 list-decimal pl-4">
              ...
              <li>Copy the message the bot replies with</li>
              <li>Paste it above and click Sign Message</li>
